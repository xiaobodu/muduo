...
static void DefaultMessageCallback(const UdpClientSocketPtr& socket, Buffer* buf, Timestamp receiveTime) {
    (void)buf;
    LOG_DEBUG << "receive udp message";
}

static void DefaultConnectCallback(const UdpClientSocketPtr& socket, UdpClientSocket::ConnectResult result) {
    LOG_DEBUG << "connect result " << result;
}

static void DefaultDisconnectCallback(const UdpClientSocketPtr& socket, UdpClientSocket::DisconnectResutl result) {
...
void UdpClientSocket::connectInLoop(const InetAddress& address) {
    ...
    if (result < 0) {
        ...
        connectCallback_(shared_from_this(), kConnectFailure);
    }
    ...
    connectCallback_(shared_from_this(), kConnectSuccess);
}
...
void UdpClientSocket::disconnectInLoop() {
    ...
    disconnectCallback_(shared_from_this(), kDisconnectSuccess);
    connectCallback_ = DefaultConnectCallback;
    messageCallback_ = DefaultMessageCallback;
    disconnectCallback_ = DefaultDisconnectCallback;
}
...
void UdpClientSocket::HandleRead(Timestamp receiveTime) {
    ...
    if (nr < 0) {
        ...
        if (messageCallback_) {
            messageCallback_(shared_from_this(), &readBuf_, receiveTime);
        }
