...
const IUINT32 IKCP_OVERHEAD = 24 - 4 - 2;
...
IUINT32 ikcp_flush(ikcpcb *kcp)
{
	...
	for (p = kcp->snd_buf.next; p != &kcp->snd_buf; p = p->next) {
		...
		if (segment->resendts < tm_packet) {
			tm_packet = segment->resendts;
		}
	}
	...
	return tm_packet > kcp->ts_flush ? kcp->ts_flush : tm_packet;
}
...
//---------------------------------------------------------------------
IUINT32 ikcp_update(ikcpcb *kcp, IUINT32 current)
{
	IINT32 slap;
